...
def train_network(opt, data_loader, model1, model2, model3, device):
    ...
    for step in tqdm(range(len(data_loader.data_loader))):
        ...
        image = inputs['image'].to(device)                         # [b, 3, w, h]

        cloth_norm = inputs['cloth_norm'].to(device)               # [b, 3, w, h]
        cloth = inputs['cloth'].to(device)                         # [b, 3, w, h]
        mloth = inputs['mloth'].to(device)                         # [b, 3, w, h]
        skeleton = inputs['skeleton'].to(device)                   # [b, 1, w, h]
        densepose = inputs['densepose'].to(device)                 # [b, 1, w, h]
        parse_preserve = inputs['parse_preserve'].to(device)       # [b, 1, w, h]
        ...
        if not opt.only_warping:
            # ================= step2 - parse7_t =================
            parse1_s = inputs['parse1_s'].to(device)                 # [b, 3, h, w]
            parse7_s = inputs['parse7_s'].to(device)                 # [b, 3, h, w]   
            hand_left_mask = inputs['hand_left_mask'].to(device)
            hand_right_mask = inputs['hand_right_mask'].to(device)
            ...
            parse1_t = Parse_7_to_1(parse7_t, device)
            ...
            # save results
            save_images(Parse_7_to_1(parse7_t, device), im_name, dir_paths["parse"], type="parse")
            save_images(limb_w, im_name, dir_paths['limb_w'])
            save_images(limb_r, im_name, dir_paths['limb_r'])

            show = torch.cat((cloth, image, to_show(warp_mloth), warp_cloth, to_show(Parse_7_to_1(parse7_t, device)), to_show(limb_w), limb_r), axis=3)
