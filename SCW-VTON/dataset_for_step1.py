...
class Dataset(data.Dataset):
    def __init__(self, opt):
        ...
        # Always use 'train' directory for all file lookups
        self.data_path = osp.join(opt.dataroot, 'train')
...
if __name__ == "__main__":
    ...
    print(f"Processing {len(dataset)} total samples...")
    print("Processing first 3 samples for testing:")
    ...
    for step, inputs in enumerate(data_loader.data_loader):
        ...
        image = inputs['image']                                    # [b, 3, w, h]
        
        print(f"Sample {step + 1}:")
        print(f"  Person image: {im_name[0]}")
        print(f"  Clothing: {c_name[0]}")
