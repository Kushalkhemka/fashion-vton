...
def refine_res(dst, name, dataroot, mode='test', pair_mode=None, pre_data_dir=None):
    # src
    # Always use train/image for source image
    src_path = os.path.join(dataroot, "train", "image", name)
    ...
    # parse
    # Always use train/image-parse-v3 for source parse
    parse_s = Image.open(os.path.join(dataroot, "train", "image-parse-v3", name.replace("jpg", "png"))).convert("L").resize((384, 512))
    parse_s = np.array(parse_s)
    parse_t = Image.open(os.path.join(pre_data_dir, "parse", name)).convert("L").resize((384, 512))
