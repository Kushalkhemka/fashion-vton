...
# from pytorch_lightning import seed_everything  # Removed due to compatibility issues
...
def load_model_from_config(config, ckpt_dir, device, verbose=False):
...
def main():
    ...
    # Use torch.manual_seed instead of seed_everything
    ...
    # Create a custom dataset class that uses our temporary pairs file
    class TempVITONDataset(VITONDataset):
        def __init__(self, dataroot, height, mode='test', pair_mode='unpaired', pre_data_dir='results'):
            super().__init__(dataroot, height, mode, pair_mode=pair_mode, pre_data_dir=pre_data_dir)
            # Override the pairs file to use our temporary one
            self.pairs_file = os.path.join(dataroot, "test_pairs_temp.txt")
            self.im_names = []
            c_names = []
            ...
            self.c_names = {'paired': self.im_names, 'unpaired': c_names}

    dataset = TempVITONDataset(opt.dataroot, opt.H, mode='test', pair_mode=opt.pair_mode, pre_data_dir=opt.pre_data_dir)
    loader = DataLoader(dataset, batch_size=opt.n_samples, shuffle=False, num_workers=0, pin_memory=False)  # Reduced workers for CPU
        ...
        with precision_scope():
            with model.ema_scope():
                for data in iterator:
                    ...
                    c = model.get_learned_conditioning(ref_tensor.to(ref_dtype))
                    c = model.proj_out(c)
